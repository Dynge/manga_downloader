...
def checkForNewChapter(chapter_list, manga_name):
    ...
    if os.path.exists(manga_documentation_file):
        ...
        downloaded_chapters = list(
            map(lambda chapter: re.sub(r"\n$", "", chapter), f.readlines())
        )
...
def download_chapters(manga_title, chapters, chunks):
    ...
    for i, chapter in enumerate(chapters):
        logger.info(
            "Downloading %s. %s chapters remaining"
            % (chapter[1], len(chapters) - (i + 1))
        )
        current_chapter_number = int(re.sub(regex_chapter_number, "", chapter[1]))
        chapter_start, chapter_end = determineStartEndChapters(
            chunk_chapter_indexs, chapters, chunk_index
        )
...
if __name__ == "__main__":
    ...
    match_index = input(
        ...
        % (
            len(search_matches),
            [(i, match[1]) for i, match in enumerate(search_matches)],
        )
    )
...
Insert amount of chunks: "
    ...
    logger.info("Finished downloading the chapters for the manga: %s." % MANGA_TITLE)
