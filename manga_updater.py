...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Script to update your kindle mangas.")
    parser.add_argument(
        "-a", "--all", action="store_true", help="Used to automatically update all."
    )
    parser.add_argument(
        "-cs",
        "--chunk_size",
        ...
        type=int,
        nargs=1,
        help="Used to specify chunk size for all updateable mangas.",
    )
    ...
    mangas_to_update = [
        re.sub(r".log", "", manga_log) for manga_log in os.listdir(md.LOG_FOLDER)
    ]
    ...
    exact_matches = [
        ...
        for match in manga
        if match[1] in mangas_to_update
    ]
    logger.debug("Exact Matches \n%s" % exact_matches)

    chapters_pr_manga = [
        md.listChapters(md.SOURCE_LINK + manga[0]) for manga in exact_matches
    ]
    ...
    for i, manga in enumerate(exact_matches):
        ...
        if len(new_chapters) > 0:
            logger.info(
                "%s new chapter(s) for manga titled: %s" % (len(new_chapters), manga[1])
            )
    ...
    if args.all == True:
        for manga, new_chapters in updateable_mangas:
            ...
            if not chunks:
                chunks = int(
                    input(
                        "%s new chapter(s) for manga titled: %s\nSelect the amount of chunks to split this into: "
                        % (len(new_chapters), manga)
                    )
                )
            logger.info(
                "Downloading %s chapter(s) for manga titled: %s\n"
                % (len(new_chapters), manga)
            )
            md.download_chapters(manga, new_chapters, chunks)
    else:
        manga_index_str = input(
            "Select the manga(s) to update by space-separated index: \n%s\n"
        ...
        for index in manga_index:
            ...
            if not chunks:
                chunks = int(
                    input(
                        "%s new chapter(s) for manga titled: %s\nSelect the amount of chunks to split this into: "
                        % (
                            len(updateable_mangas[index][1]),
                            updateable_mangas[index][0],
                        )
                    )
                )
            logger.info(
            ...
            md.download_chapters(
                updateable_mangas[index][0], updateable_mangas[index][1], chunks
            )
